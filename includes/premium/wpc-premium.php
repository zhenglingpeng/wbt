...
if ( ! function_exists( 'woobt_update_checker' ) ) {
	add_action( 'init', 'woobt_update_checker', 99 );
	add_action( 'admin_notices', 'woobt_update_notice', 99 );
	add_action( 'in_plugin_update_message-' . plugin_basename( WOOBT_FILE ), 'woobt_update_license', 99 );

	function woobt_update_checker() {
		if ( $key = wpc_get_update_key( 6141 ) ) {
			PucFactory::buildUpdateChecker( 'https://api.wpclever.net/update/' . $key . '.json', WOOBT_FILE, plugin_basename( WOOBT_DIR ) );
		} else {
			PucFactory::buildUpdateChecker( 'https://api.wpclever.net/update/6141.json', WOOBT_FILE, plugin_basename( WOOBT_DIR ) );
		}
	}

	function woobt_update_license( $plugin_data ) {
	...
	function woobt_update_notice() {
		if ( apply_filters( 'wpc_dismiss_notices', false ) ) {
			return;
		}

		if ( get_option( 'wpc_dismiss_notice_woobt_update' ) ) {
			return;
		}

		if ( ! wpc_get_update_key( 6141 ) ) {
			?>
            <div data-dismissible="woobt_update" class="wpc-notice notice notice-warning is-dismissible">
                <p>Please verify
                    <a href="<?php echo esc_url( admin_url( 'admin.php?page=wpclever-keys' ) ); ?>">License Key</a> of
                    <strong>WPC Frequently Bought Together</strong> to enjoy unlimited update release and get the latest
                    plugin update directly on the website backend.
                </p>
            </div>
			<?php
		}
...
if ( ! class_exists( 'WPCleverPremium' ) ) {
	class WPCleverPremium {
		...
		function admin_enqueue_scripts( $hook ) {
			if ( strpos( $hook, 'wpclever-keys' ) ) {
				wp_enqueue_style( 'wpc-premium', WOOBT_URI . 'includes/premium/css/premium.css' );
				wp_enqueue_script( 'wpc-premium', WOOBT_URI . 'includes/premium/js/premium.js', [ 'jquery', ] );
			}
		}
					...
					if ( ! empty( $keys ) ) {
                        ...
                        <table class="wpc_update_keys">
							...
							foreach ( array_reverse( $keys ) as $key => $val ) {
								...
								echo '</ul>';
								echo '</td>';
								echo '<td><div class="wpc-premium-validated">Validated on: ' . esc_html( isset( $val['date'] ) ? $val['date'] : '' ) . '</div><div class="wpc-premium-support-expires">Support expires: ' . ( isset( $val['date'] ) ? esc_html( wp_date( 'Y-m-d H:i:s', strtotime( '+1 year', strtotime( $val['date'] ) ) ) ) : '' ) . ' ' . ( time() > strtotime( '+1 year', strtotime( $val['date'] ) ) ? '(expired)' : '' ) . '</div></td>';
